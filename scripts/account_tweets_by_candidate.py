...
def get_chart_for_candidate(candidate, candidate_dict, sdate, edate, delta, candidate_accounts):
    ...
    for i in range(0, len(candidate_accounts)):
        ...
        retweet_list = candidate_dict[candidate_accounts[i]]['Retweet count']

        plt.scatter(dates, count_list, s=5, )
        plt.xlabel("Date")
        plt.ylabel("Number of tweets")
        plt.title("Number of tweets for date for accounts connected to candidate " + candidate)
        plt.plot(dates, count_list, label=candidate_accounts[i], )
    ...
    for i in range(0, len(candidate_accounts)):
        ...
        plt.title("Favorite count for date for accounts connected to candidate " + candidate)
        plt.plot(dates, fav_list, label=candidate_accounts[i], )
    ...
    for i in range(0, len(candidate_accounts)):
        ...
        plt.title("Retweets count for date for accounts connected to candidate " + candidate)
        plt.plot(dates, retweet_list, label=candidate_accounts[i], )
